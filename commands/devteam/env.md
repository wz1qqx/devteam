---
name: devteam:env
description: Environment registry — list profiles/environments, bind runtime, plan bootstrap, doctor, or refresh remote/k8s profiles
argument-hint: "<list|show|environments|doctor|runtime|bind|bootstrap|refresh> [--root <path>] [--set <track>] [--feat <feat>] [--env <environment>] [--profile <name>] [--sync <profile>] [--remote] [--yes] [--run <id>] [--text]"
allowed-tools:
  - Read
  - Bash
  - Glob
...
---
<objective>
Inspect machine/cluster environments, bind runtime state, plan initial environment bootstrap, and refresh vLLM editable remote venvs when explicitly requested.
</objective>
