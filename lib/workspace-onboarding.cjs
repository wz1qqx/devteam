...
const { bindTrack, listTracks, readSelectionBinding, trackStatus } = require('./track-profile.cjs');
const { bindRuntimeContext, buildRuntimeContext, readRuntimeBinding } = require('./runtime-context.cjs');
...
function resolveContextSelection(config, options = {}) {
  const explicit = options.set || options.feat;
  const envSelected = process.env.DEVTEAM_TRACK || process.env.DEVTEAM_FEAT || process.env.DEVTEAM_FEATURE;
  const binding = readSelectionBinding({
    config,
    scope: options.scope || options.selectionScope || 'session',
  });
  const bindingUsable = !explicit && !envSelected && binding.exists && binding.track && !binding.error;
  const selection = resolveTrackSelection(config, {
    set: bindingUsable ? binding.track : (options.set || null),
    feat: bindingUsable ? binding.feat : (options.feat || null),
    required: false,
    default: false,
    featDefault: false,
  });
  if (bindingUsable) {
    selection.track_source = `binding:${binding.scope}`;
    selection.feat_source = selection.feat ? `binding:${binding.scope}` : 'none';
    selection.source = selection.track_source;
    selection.track_input = binding.track;
    selection.feat_input = binding.feat || null;
  }
  return { selection, binding };
}

function safeRuntimeSummary(config, selected, selectedFeat) {
  if (!selected) return null;
  try {
    ...
    const binding = readRuntimeBinding({
      config,
      context: runtime,
    });
    return {
      ...
      env_count: Object.keys(runtime.env || {}).length,
      worktree_count: Array.isArray(runtime.worktrees) ? runtime.worktrees.length : 0,
      binding,
      bind_command: [
        'node',
        shellArg(devteamCliPath()),
        'env bind',
        '--root',
        shellArg(config.root),
        '--set',
        shellArg(selected),
        selectedFeat ? '--feat' : null,
        selectedFeat ? shellArg(selectedFeat) : null,
        runtime.profile ? '--profile' : null,
        runtime.profile ? shellArg(runtime.profile) : null,
        runtime.sync_profile ? '--sync' : null,
        runtime.sync_profile ? shellArg(runtime.sync_profile) : null,
        runtime.environment ? '--env' : null,
        runtime.environment ? shellArg(runtime.environment) : null,
        '--text',
      ].filter(Boolean).join(' '),
    };
  } catch (err) {
...
function safeBootstrapSummary(config, selected, selectedFeat, runtimeSummary) {
  if (!selected || !runtimeSummary || runtimeSummary.error || !runtimeSummary.profile) return null;
  try {
    const plan = envBootstrapPlan({
      root: config.root,
      set: selected,
      feat: selectedFeat || null,
      profile: runtimeSummary.profile,
      default: false,
      featDefault: false,
    });
    return {
      status: plan.status,
      recipe: plan.recipe,
      command_count: Array.isArray(plan.commands) ? plan.commands.length : 0,
...
function workspaceActivate(options = {}) {
  const config = loadWorkspaceConfig(options.root || null);
  const selection = resolveTrackSelection(config, {
    set: options.set || options.track || null,
    feat: options.feat || null,
    required: true,
    default: false,
    featDefault: false,
    label: 'workspace activate track',
  });
  const selectionBinding = bindTrack({
    root: config.root,
  ...
  const runtimeBinding = bindRuntimeContext({
    config,
    set: selection.track,
    feat: selection.feat || null,
  ...
  const sources = [
    selectionBinding.binding ? selectionBinding.binding.source : null,
  ...
  return {
    ...
    sources,
    next_action: 'Source the selection and runtime binding files in any shell that should use this workspace state.',
  };
}
...
function workspaceContext(options = {}) {
  ...
  const { selection, binding: selectionBinding } = resolveContextSelection(config, options);
  ...
  const runtime = safeRuntimeSummary(config, selected, selectedFeat);
  const bootstrap = safeBootstrapSummary(config, selected, selectedFeat, runtime);
  ...
  const primary = selectedRuntime.next_actions && selectedRuntime.next_actions[0]
    ...
    : {
      ...
      summary: selected
        ...
        : 'Bind or choose a track and optional feature before editing code.',
      command: selected
  ...
  return {
    ...
    policy: {
      track_selection: 'session-local; prefer .devteam/state/selection-*.sh, --set <track>, or DEVTEAM_TRACK over mutating workspace defaults',
      mutation: 'do not sync, publish, build, deploy, or mutate remote state without explicit user intent',
      evidence: 'run evidence is optional; record it only when a handoff or release decision needs durable proof',
    },
    ...
    recommended_commands: {
      ...
      track_context: selected
        ? command(['track', 'context', '--root', shellArg(config.root), ...selectedScopeArgs, '--text'])
        : null,
      selection_bind: selected
        ? command(['track', 'bind', shellArg(selected), '--root', shellArg(config.root), selectedFeat ? '--feat' : null, selectedFeat ? shellArg(selectedFeat) : null, '--write', '--text'].filter(Boolean))
        : null,
      runtime_bind: selected && runtime && runtime.bind_command ? runtime.bind_command : null,
      status: selected
...
function renderWorkspaceContextText(data) {
  ...
  lines.push(
    ...
    '  - Bind a track and optional feature before editing code.',
    '  - Prefer the session-local selection/runtime binding files over changing workspace defaults.',
    '  - Use run evidence only when a handoff or release decision needs durable proof.',
  ...
  if (data.selection_binding && data.selection_binding.exists) {
    lines.push(`  selection_binding: ${data.selection_binding.source}`);
  }
  if (data.runtime) {
    if (data.runtime.error) {
      lines.push(`  runtime_warning: ${data.runtime.error}`);
    } else {
      const runtimeBinding = data.runtime.binding || {};
      lines.push(
        '',
        'Runtime binding:',
        `  profile: ${data.runtime.profile || '-'} (${data.runtime.type || '-'}) env=${data.runtime.environment || '-'} proxy=${data.runtime.proxy_configured ? 'yes' : 'no'}`,
        `  exports: ${data.runtime.env_count || 0} worktrees=${data.runtime.worktree_count || 0}`,
        `  binding: ${runtimeBinding.exists ? (runtimeBinding.current ? 'current' : 'stale') : 'none'}`,
        runtimeBinding.exists ? `  source: ${runtimeBinding.source}` : `  bind: ${data.runtime.bind_command}`,
      );
    }
  }
  if (data.bootstrap) {
    if (data.bootstrap.error) {
      lines.push(`  bootstrap_warning: ${data.bootstrap.error}`);
    } else {
      lines.push(`  bootstrap: ${data.bootstrap.status}${data.bootstrap.recipe ? ` recipe=${shortPath(data.bootstrap.recipe.abs_path || data.bootstrap.recipe.path)}` : ''} commands=${data.bootstrap.command_count || 0}`);
    }
  }
  ...
  lines.push(
    ...
    data.recommended_commands.selection_bind ? `  ${data.recommended_commands.selection_bind}` : null,
    data.recommended_commands.runtime_bind ? `  ${data.recommended_commands.runtime_bind}` : null,
  );
...
function renderWorkspaceActivateText(result) {
  const lines = [
    'Workspace Activate',
    '',
    `Workspace: ${shortPath(result.workspace)}`,
  ...
  if (result.selection_binding) {
    lines.push(
      '',
      `Selection: ${shortPath(result.selection_binding.shell_path)}`,
    );
  }
  if (result.runtime_binding) {
    lines.push(`Runtime: ${shortPath(result.runtime_binding.shell_path)}`);
  }
  lines.push('', `Next: ${result.next_action}`);
  return lines.join('\n');
}
...
function handleWorkspaceOnboarding(subcommand, args) {
  ...
  if (subcommand === 'activate') {
    const result = workspaceActivate({
      root: parsed.root || null,
      set: parsed.set || parsed.track || null,
      feat: parsed.feat || null,
      scope: parsed.scope || null,
    ...
    if (parsed.text === true) process.stdout.write(renderWorkspaceActivateText(result) + '\n');
    else output(result);
    return;
  }
  ...
  error(`Unknown workspace subcommand: '${subcommand}'. Use: scaffold, onboard, context, activate`);
}
