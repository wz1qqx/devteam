...
function resolveHarnessSelection(config, options = {}, selectionBinding = null) {
  ...
  const bindingUsable = !explicit && !envSelected && selectionBinding && selectionBinding.exists && selectionBinding.track && !selectionBinding.error;
  const selection = resolveTrackSelection(config, {
    set: bindingUsable ? selectionBinding.track : (options.set || null),
    feat: bindingUsable ? selectionBinding.feat : (options.feat || null),
    required: false,
    default: !bindingUsable,
    featDefault: !bindingUsable,
  });
  if (bindingUsable) {
    selection.track_source = `binding:${selectionBinding.scope}`;
    selection.feat_source = selection.feat ? `binding:${selectionBinding.scope}` : 'none';
    selection.source = selection.track_source;
    selection.track_input = selectionBinding.track;
    selection.feat_input = selectionBinding.feat || null;
  }
  return selection;
}
...
function harnessStatus(options = {}) {
  ...
  const selection = resolveHarnessSelection(config, options, selectionBinding);
  ...
  const payload = {
    ...
    workspace_set_source: selection.track_source || worktrees.workspace_set_source || null,
    selection,
