...
function buildNextActions(status) {
  ...
  if (status.runtime && status.runtime.shell && status.runtime.shell.source) {
...
function renderHarnessStatusText(status) {
  ...
  const lines = [
    ...
    `Runtime: ${status.runtime.env_keys.length} exports, ${status.runtime.worktrees.length} worktree bindings`,
