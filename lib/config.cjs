...
const { output, error, findWorkspaceRoot, expandHome, DevflowError } = require('./core.cjs');
const { FEATURE_PHASES } = require('./stage-constants.cjs');
const yaml = require('./yaml.cjs');

const SUPPORTED_SHIP_STRATEGIES = new Set(['k8s']);
...
function loadSplitConfig(root, wsPath) {
  ...
  try {
    ...
    if (e instanceof DevflowError) throw e;
...
function normalizeFeatureConfig(feature, featureName) {
  ...
  for (const [repoName, scopeEntry] of Object.entries(scope)) {
    ...
    const { shared_with: _deprecatedSharedWith, ...scopeEntryWithoutSharedWith } = scopeEntry;
    normalizedScope[repoName] = {
      ...scopeEntryWithoutSharedWith,
      dev_slot: devSlot,
  ...
  const shipStrategy = ship.strategy;
...
function getFeatureRepos(config, featureName) {
  ...
  for (const [repoName, scopeEntry] of Object.entries(feature.scope)) {
    ...
    const devWorktreePath = devSlot
      ? devSlot.worktree
      : (scopeEntry.dev_worktree || null);
