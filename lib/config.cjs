...
function loadConfig(workspaceRoot) {
  ...
  if (!yamlContent.trim()) {
    error('.dev.yaml is empty. Run /devteam:init workspace to initialize it.');
  }
  try {
    const raw = yaml.parse(yamlContent);
    if (!raw || typeof raw !== 'object') {
      error('.dev.yaml is empty or invalid. Run /devteam:init workspace to initialize it.');
    }
    const config = { _root: root, _path: yamlPath, ...raw };

    if (config.schema_version !== 2) {
      error('Unsupported schema_version. Only v2 is supported. Migrate with /devteam:init workspace.');
    }
    return config;
  } catch (err) {
