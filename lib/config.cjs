...
const DEPLOY_PROFILE_TYPES_K8S = new Set(['k8s']);
const DEPLOY_PROFILE_TYPES_BARE = new Set(['bare_metal_venv', 'bare_metal_docker']);
...
/**
 * Legacy field ship.metal.build_mode is deprecated for the build stage — authoritative value is build.mode
 * (orchestrator --build-mode overrides build.mode). Loader merges legacy metal.build_mode into build.mode
 * when build.mode is absent, then mirrors build.mode back onto ship.metal.build_mode for bare_metal.
 */
function normalizeShipMetal(metal, featureName) {
  ...
  return {
    ...
    deploy_profile: metal.deploy_profile || null,
...
/**
 * Authoritative build stage mode: feature.build.mode.
 * If missing, legacy ship.metal.build_mode is merged in (then mirrored back onto metal).
 */
function normalizeFeatureBuild(buildRaw, shipMetal, shipStrategy, featureName) {
  const build = normalizeOptionalMapping(buildRaw, `Feature '${featureName}' build`);
  let mode = build.mode != null ? build.mode : null;
  if (mode == null && shipMetal && shipMetal.build_mode != null) {
    mode = shipMetal.build_mode;
  }
  if (shipStrategy === 'bare_metal' && mode == null) {
    mode = 'sync_only';
  }
  if (mode != null && !SUPPORTED_BARE_METAL_BUILD_MODES.has(mode)) {
    error(
      `Feature '${featureName}' build.mode '${mode}' is invalid. ` +
      `Valid values: ${Array.from(SUPPORTED_BARE_METAL_BUILD_MODES).join(', ')}`
    );
  }
  const out = { ...build };
...
/**
 * Single-field switch: deploy.active_profile selects deploy.profiles[key] and derives ship.strategy
 * plus deploy.deploy_profile (k8s) or ship.metal.deploy_profile (bare metal).
 */
function resolveDeployActiveProfile(deploy, ship, featureName) {
  const d = deploy && typeof deploy === 'object' ? { ...deploy } : {};
  const s = ship && typeof ship === 'object' ? { ...ship } : {};
  if (d.active_profile == null || d.active_profile === '') {
    return { deploy: d, ship: s };
  }
  const key = d.active_profile;
  if (typeof key !== 'string') {
    error(`Feature '${featureName}' deploy.active_profile must be a string.`);
  }
  const profiles = d.profiles;
  if (!profiles || typeof profiles !== 'object' || Array.isArray(profiles)) {
    error(`Feature '${featureName}' deploy.active_profile requires deploy.profiles to be a mapping.`);
  }
  const prof = profiles[key];
  if (!prof || typeof prof !== 'object') {
    error(`Feature '${featureName}' deploy.active_profile '${key}' not found in deploy.profiles`);
  }
  const ptype = prof.type;
  if (!ptype || typeof ptype !== 'string') {
    error(`Feature '${featureName}' deploy.profiles.${key}.type is required`);
  }
  let derived = null;
  if (DEPLOY_PROFILE_TYPES_K8S.has(ptype)) derived = 'k8s';
  else if (DEPLOY_PROFILE_TYPES_BARE.has(ptype)) derived = 'bare_metal';
  else {
    error(
      `Feature '${featureName}' deploy.profiles.${key}.type '${ptype}' is unsupported. ` +
      `Use: k8s, bare_metal_venv, or bare_metal_docker`
    );
  }
  const explicit = s.strategy || null;
  if (explicit && explicit !== derived) {
    error(
      `Feature '${featureName}' deploy.active_profile '${key}' implies ship.strategy '${derived}' ` +
      `but ship.strategy is '${explicit}'`
    );
  }
  s.strategy = derived;
  if (ptype === 'k8s') {
    d.deploy_profile = key;
  } else {
    s.metal = s.metal && typeof s.metal === 'object' ? { ...s.metal } : {};
    s.metal.deploy_profile = key;
  }
  return { deploy: d, ship: s };
}
...
function normalizeFeatureConfig(feature, featureName) {
  ...
  let deploy = normalizeOptionalMapping(feature.deploy, `Feature '${featureName}' deploy`);
  let ship = normalizeOptionalMapping(feature.ship, `Feature '${featureName}' ship`);
  ({ deploy, ship } = resolveDeployActiveProfile(deploy, ship, featureName));

  let shipStrategy = ship.strategy || null;
  ...
  const buildMerged = normalizeFeatureBuild(feature.build, ship.metal, shipStrategy, featureName);
  if (shipStrategy === 'bare_metal' && ship.metal) {
    ship.metal.build_mode = buildMerged.mode;
  }
