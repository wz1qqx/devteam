...
const { loadConfig, getActiveFeatureWithRepos, getActiveCluster, getFeatureRepos, listFeatures, listFeatureNames } = require('./config.cjs');
...
async function initWorkflow(workflowName, args) {
  ...
  if (!featureWithRepos) {
    const available = listFeatureNames(config);
    output({
      workspace,
      vault,
  ...
  const workflowContextMap = {
    ...
    'team-vllm-opt': { ...base, cluster: clusterCtx, build_history: buildHistory, benchmark: featureWithRepos.benchmark || {}, verify: featureWithRepos.verify || {}, deploy: featureWithRepos.deploy || {}, knowledge_notes: knowledgeNotes, devlog: config.devlog || {} },
    // Retained independent skills
