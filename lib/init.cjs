...
const { loadConfig, requireFeature, resolveFeatureWithRepos, getActiveCluster, getFeatureRepos, listFeatures, listFeatureNames, getWorkspaceConfigPath } = require('./config.cjs');
...
async function initWorkflow(workflowName, args) {
  ...
  const deployConfig = featureWithRepos ? featureWithRepos.deploy : {};
  ...
  const workflowContextMap = {
    // === Team orchestration ===
    'team':         { ...base, ...memoryContext, cluster: clusterCtx, all_clusters: buildClusterSummary(config), build_history: buildHistory, build: buildConfig, deploy: deployConfig, build_server: config.build_server, benchmark: benchmarkConfig, verify: verifyConfig, knowledge_notes: knowledgeNotes, feature_state: featureState, devlog: config.devlog },
    ...
    'team-build':   { ...base, cluster: clusterCtx, build_history: buildHistory, build: buildConfig, build_server: config.build_server, knowledge_notes: knowledgeNotes, devlog: config.devlog },
    'team-deploy':  { ...base, cluster: clusterCtx, all_clusters: buildClusterSummary(config), deploy: deployConfig, build_server: config.build_server },
    'team-verify':  { ...base, cluster: clusterCtx, build_history: buildHistory, benchmark: benchmarkConfig, verify: verifyConfig, deploy: deployConfig, build_server: config.build_server, knowledge_notes: knowledgeNotes, devlog: config.devlog },
