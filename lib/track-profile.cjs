...
function safeFileToken(value, fallback = 'selection') {
...
function selectionBindingPaths(config, scope) {
  const safeScope = safeFileToken(scope || 'session');
  const stateDir = path.join(config.root, WORKSPACE_DIR, 'state');
  return {
    scope: safeScope,
    shell_path: path.join(stateDir, `selection-${safeScope}.sh`),
    json_path: path.join(stateDir, `selection-${safeScope}.json`),
  };
}

function renderSelectionShell(binding) {
  const lines = [
    '#!/usr/bin/env bash',
    '# Generated by devteam. Source this to select a track/feature for this shell.',
    '',
    `export DEVTEAM_TRACK=${shellQuote(binding.track)}`,
  ];
  if (binding.feat) {
    lines.push(`export DEVTEAM_FEAT=${shellQuote(binding.feat)}`);
  } else {
    lines.push('unset DEVTEAM_FEAT');
  }
  return `${lines.join('\n')}\n`;
}

function writeSelectionBinding(config, binding) {
  ensureWorkspaceDirs(config.root);
  fs.writeFileSync(binding.shell_path, renderSelectionShell(binding), 'utf8');
  fs.writeFileSync(binding.json_path, `${JSON.stringify(binding, null, 2)}\n`, 'utf8');
}

function readSelectionBinding(options = {}) {
  const config = options.config || loadWorkspaceConfig(options.root || null);
  const scope = options.scope || 'session';
  const paths = selectionBindingPaths(config, scope);
  const summary = {
    exists: fs.existsSync(paths.shell_path),
    scope: paths.scope,
    shell_path: paths.shell_path,
    json_path: paths.json_path,
    source: `. ${shellQuote(paths.shell_path)}`,
    shell_exists: fs.existsSync(paths.shell_path),
    json_exists: fs.existsSync(paths.json_path),
    track: null,
    feat: null,
    bound_at: null,
    error: null,
  };
  if (!summary.json_exists) return summary;
  try {
    const record = JSON.parse(fs.readFileSync(paths.json_path, 'utf8'));
    summary.track = record.track || null;
    summary.feat = record.feat || null;
...
function bindTrack(options = {}) {
  ...
  const profile = inferTrackProfile(config, track, { activeTrack: track, feat: selection.feat || null });
  const exports = {
    DEVTEAM_TRACK: track,
  };
  if (selection.feat) exports.DEVTEAM_FEAT = selection.feat;
  const commands = [
    `export DEVTEAM_TRACK=${shellArg(track)}`,
    selection.feat ? `export DEVTEAM_FEAT=${shellArg(selection.feat)}` : 'unset DEVTEAM_FEAT',
  ];
  const result = {
    action: 'track_bind',
    workspace: config.root,
    ...
    wrote_binding: false,
    binding: null,
    next_action: 'Run the export command in this terminal/session, pass --set explicitly, or use --write to create a stable .devteam/state selection binding. This does not modify .devteam/config.yaml.',
  };
  if (options.write === true) {
    const paths = selectionBindingPaths(config, options.scope || 'session');
    const binding = {
      action: 'selection_bind',
      workspace: config.root,
      scope: paths.scope,
      shell_path: paths.shell_path,
      json_path: paths.json_path,
    ...
    writeSelectionBinding(config, binding);
    result.wrote_binding = true;
    result.binding = binding;
    result.next_action = `Source ${binding.shell_path} in this terminal/session. This does not modify .devteam/config.yaml.`;
  }
  return result;
}
  ...
  if (subcommand === 'bind') {
    ...
    if (parsed.text === true) {
      process.stdout.write([
        ...
        result.binding ? `Source: ${result.binding.source}` : null,
