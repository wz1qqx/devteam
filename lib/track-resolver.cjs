...
function inferTrackProfile(config, track, options = {}) {
  ...
  const validationName = entry.validation || findProfile(config, 'validation', name);
  const validation = validationName ? (config.validation_profiles[validationName] || {}) : null;
  const buildName = entry.build || findProfile(config, 'build', name);
  const deployFlowName = findDeployFlow(config, name);
  const deployFlow = deployFlowName ? (config.deploy_flows[deployFlowName] || {}) : null;
  const preferredEnv = entry.env || (validation && validation.env ? String(validation.env) : null);
  ...
  const sync = entry.sync || (syncProfiles.length === 1 ? syncProfiles[0] : (env || (config.defaults ? config.defaults.sync : null) || null));
  ...
  return {
    ...
    sync,
    build: buildName,
    deploy: entry.deploy || (deployFlow && deployFlow.profile ? String(deployFlow.profile) : null),
    deploy_flow: deployFlowName,
    validation: validationName,
    server_test: entry.server_test || null,
