...
function runHooks(root, featureName, options = {}) {
  ...
  const repos = resolveHookRepos(config, feature.name, runState);
...
function resolveHookRepos(config, featureName, runState) {
  if (runState && Array.isArray(runState.repos) && runState.repos.length > 0) {
  ...
  return getFeatureRepos(config, featureName);
}
