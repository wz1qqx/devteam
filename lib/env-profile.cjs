...
function localDirtySignature(worktreePath) {
  ...
  const status = gitLocal(worktreePath, ['status', '--porcelain=v1', '-z'], { raw: true });
  if (!status) return null;
  const diff = gitLocal(worktreePath, ['diff', '--binary', 'HEAD', '--'], { raw: true }) || '';
  const untrackedRaw = gitLocal(worktreePath, ['ls-files', '--others', '--exclude-standard', '-z'], { raw: true }) || '';
  const untracked = [];
...
function localGitSourceStatus(localPath, options = {}) {
  ...
  source.dirty_signature = source.dirty ? localDirtySignature(localPath) : null;
  return source;
}
...
function remoteSourceStatusCommand(remotePath, dirtyLimit = 20, options = {}) {
  const limit = Number.isInteger(Number(dirtyLimit)) && Number(dirtyLimit) >= 0
    ? Number(dirtyLimit)
    : 20;
  const venv = options.venv ? String(options.venv) : '';
  const python = options.python ? String(options.python) : '';
  return [
    'set -u',
    ...
    'printf "dirty_count=%s\\n" "$(git status --porcelain=v1 --untracked-files=all -- . ":(exclude).devteam-sync-binding.json" 2>/dev/null | wc -l | tr -d " ")"',
    'printf "binding_json="; if [ -f .devteam-sync-binding.json ]; then tr -d "\\n" < .devteam-sync-binding.json; fi; printf "\\n"',
    venv ? `VENV=${shellQuote(venv)}` : null,
    venv ? 'printf "venv_path=%s\\n" "$VENV"' : 'printf "venv_path=\\n"',
    venv ? 'if [ -d "$VENV" ]; then echo venv_exists=1; else echo venv_exists=0; fi' : null,
    venv ? 'printf "venv_binding_json="; if [ -f "$VENV/.devteam-venv-binding.json" ]; then tr -d "\\n" < "$VENV/.devteam-venv-binding.json"; fi; printf "\\n"' : null,
    python ? `PYTHON=${shellQuote(python)}` : null,
    python ? 'printf "python_path=%s\\n" "$PYTHON"' : 'printf "python_path=\\n"',
    python ? 'if [ -x "$PYTHON" ]; then echo python_exists=1; else echo python_exists=0; fi' : null,
    'echo __DEVTEAM_REMOTE_STATUS_PORCELAIN__',
    `if [ ${limit} -gt 0 ]; then git status --porcelain=v1 --untracked-files=all -- . ":(exclude).devteam-sync-binding.json" 2>/dev/null | sed -n '1,${limit}p'; fi`,
...
function parseRemoteSourceStatus(stdout, dirtyLimit = 20) {
  ...
  const binding = parseJsonField(fields.binding_json);
  const venv = fields.venv_path ? {
    path: fields.venv_path,
    exists: fields.venv_exists == null ? null : fields.venv_exists === '1',
    python: fields.python_path || null,
    python_exists: fields.python_exists == null ? null : fields.python_exists === '1',
    binding: parseJsonField(fields.venv_binding_json),
  } : null;
  return {
...
function remoteGitSourceStatus(profile, remotePath, options = {}) {
  ...
  const command = remoteSourceStatusCommand(remotePath, limit, {
    venv: includeVenv ? profile.venv || null : null,
    python: includeVenv ? profile.python || null : null,
  });
...
function sourceCompareProblems(local, remote, options = {}) {
  const problems = [];
  const remoteHead = options.remoteHead || (remote ? remote.head : null);
  const remoteBranch = options.remoteBranch || (remote ? remote.branch : null);
  ...
  if (local && local.status === 'ok' && local.dirty && options.allowLocalDirty !== true) problems.push('local_dirty');
  if (remote && remote.status === 'ok' && remote.dirty && options.allowRemoteDirty !== true) problems.push('remote_dirty');
  if (local && remote && local.status === 'ok' && remote.status === 'ok') {
    if (local.head && remoteHead && local.head !== remoteHead) problems.push('head_mismatch');
    if (local.branch && remoteBranch && local.branch !== remoteBranch) problems.push('branch_mismatch');
  }
  return problems;
}

function remoteBindingMatches(binding, profile, worktree, remotePath, options = {}) {
  if (!binding || binding.parse_error) return false;
  if (binding.profile !== profile.name) return false;
  if (binding.worktree_id !== worktree.id) return false;
  if (binding.remote_path !== remotePath) return false;
  if (options.track && binding.track !== options.track) return false;
  const feat = options.feat || null;
  if ((binding.feat || null) !== feat) return false;
  return true;
}

function venvBindingMatches(binding, profile, worktree, remotePath, options = {}) {
  if (!binding || binding.parse_error) return false;
  if (binding.profile !== profile.name) return false;
  if (binding.worktree_id !== worktree.id) return false;
  if (binding.remote_path !== remotePath) return false;
  if (profile.venv && binding.venv !== profile.venv) return false;
  if (profile.python && binding.python !== profile.python) return false;
  if (options.track && binding.track !== options.track) return false;
  const feat = options.feat || null;
  if ((binding.feat || null) !== feat) return false;
  return true;
...
function sourceCompareStatus(problems) {
  ...
  if (problems.some(problem => [
    ...
    'venv_missing',
    'venv_python_missing',
    'venv_binding_missing',
    'venv_binding_mismatch',
  ].includes(problem))) {
    return 'blocked';
  }
  return 'drift';
...
function remoteStatusEntry(profile, worktree, options = {}) {
  ...
  const bindingMatches = remoteBindingMatches(remote.binding || null, profile, worktree, remotePath, options);
  const bindingDirtyKnown = bindingMatches && remote.binding &&
    Object.prototype.hasOwnProperty.call(remote.binding, 'source_dirty');
  const bindingDirtyMatches = bindingMatches && (
    !bindingDirtyKnown ||
    (
      (remote.binding.source_dirty === true || remote.binding.source_dirty === 'true') === (local && local.dirty === true) &&
      (
        !(remote.binding.source_dirty === true || remote.binding.source_dirty === 'true') ||
        !remote.binding.source_dirty_signature ||
        remote.binding.source_dirty_signature === local.dirty_signature
      )
    )
  );
  const venvBinding = remote.venv && remote.venv.binding ? remote.venv.binding : null;
  const venvBindingMatch = venvBindingMatches(venvBinding, profile, worktree, remotePath, options);
  if (remote.venv) remote.venv.binding_matches = venvBindingMatch;
  remote.binding_matches = bindingMatches;
  remote.binding_dirty_matches = bindingDirtyMatches;
  remote.effective_head = bindingMatches && remote.binding && remote.binding.source_head
    ? remote.binding.source_head
    : remote.head;
  remote.effective_branch = bindingMatches && remote.binding && remote.binding.branch
    ? remote.binding.branch
    : remote.branch;
  const problems = sourceCompareProblems(local, remote, {
    allowRemoteDirty: bindingMatches && bindingDirtyMatches,
    allowLocalDirty: bindingMatches && bindingDirtyMatches,
    remoteHead: remote.effective_head,
    remoteBranch: remote.effective_branch,
  });
  if (bindingMatches && !bindingDirtyMatches) {
    problems.push('binding_dirty_mismatch');
  }
  if (bindingMatches && remote.binding && remote.binding.source_head && remote.head && remote.binding.source_head !== remote.head) {
    problems.push('remote_git_head_mismatch');
  }
  if (bindingMatches && remote.binding && remote.binding.branch && remote.branch && remote.binding.branch !== remote.branch) {
    problems.push('remote_git_branch_mismatch');
  }
  if (remote.venv) {
    if (remote.venv.exists === false) problems.push('venv_missing');
    if (remote.venv.python_exists === false) problems.push('venv_python_missing');
    if (remote.venv.exists === true && !venvBinding) problems.push('venv_binding_missing');
    if (venvBinding && !venvBindingMatch) problems.push('venv_binding_mismatch');
  }
  ...
  return {
    ...
    next_action: status === 'match'
      ...
      : (problems.includes('venv_binding_missing') || problems.includes('venv_binding_mismatch')
        ? 'Refresh this venv from the selected source before running remote tests.'
...
function envRemoteStatus(options = {}) {
  ...
  const entries = worktrees.map((worktree, index) => remoteStatusEntry(profile, worktree, {
    ...
    includeVenv: options.includeVenv !== false,
  ...
  const totals = entries.reduce((acc, entry) => {
    ...
    if (entry.problems.includes('remote_git_head_mismatch')) acc.remote_git_head_mismatch += 1;
    if (entry.problems.includes('remote_git_branch_mismatch')) acc.remote_git_branch_mismatch += 1;
    return acc;
  }, {
    sources: 0,
    match: 0,
    ...
    head_mismatch: 0,
    branch_mismatch: 0,
    remote_git_head_mismatch: 0,
    remote_git_branch_mismatch: 0,
  });
  const status = totals.blocked > 0
    ? 'blocked'
    : (totals.drift > 0 ? 'drift' : 'match');
  const hasVenvBindingProblem = entries.some(entry =>
    entry.problems.includes('venv_binding_missing') ||
    entry.problems.includes('venv_binding_mismatch') ||
    entry.problems.includes('venv_missing') ||
    entry.problems.includes('venv_python_missing')
  );
  return {
    action: 'env_remote_status',
    workspace: config.root,
    ...
    next_action: status === 'match'
      ...
      : (hasVenvBindingProblem
        ? 'Refresh or rebuild the selected venv after syncing the selected source mirror.'
...
function remoteStatusHasBlockingDrift(status, options = {}) {
  const allowedProblems = new Set(normalizeStringList(options.allowedProblems));
  const blocking = [];
  for (const source of status.sources || []) {
    const problems = (source.problems || []).filter(problem => {
      if (allowedProblems.has(problem)) return false;
      if (
        options.allowBoundRemoteDirty === true &&
        problem === 'remote_dirty' &&
        source.remote &&
        source.remote.binding_matches === true
      ) {
        return false;
      }
      return true;
...
function renderEnvRemoteStatusText(status) {
  ...
  if (!status.sources || !status.sources.length) {
    ...
    for (const source of status.sources) {
      ...
      if (remote.venv) {
        lines.push(`    venv:   ${remote.venv.path || '-'}  ${remote.venv.exists === false ? 'missing' : 'present'}  binding=${remote.venv.binding_matches ? 'match' : 'missing-or-mismatch'}`);
      }
...
function buildVllmRefreshCommand(profile, options = {}) {
  ...
  const createVenv = options.createVenv === true;
  ...
  const lines = [
    ...
    createVenv
      ? `(test -x ${shellQuote(python)} || ${shellQuote(uv)} venv --clear --python ${shellQuote(profile.python_version || '3.12')} --seed --managed-python ${shellQuote(venv)})`
      : null,
    ...
    binding ? `printf %s ${shellQuote(JSON.stringify(binding))} > ${shellQuote(`${venv.replace(/\/+$/, '')}/.devteam-venv-binding.json`)}` : null,
    importCheck,
  ].filter(Boolean);
  return lines.join(' && ');
}

function envRefreshRemoteGuard(config, profileName, options = {}) {
  const status = envRemoteStatus({
    config,
    profile: profileName,
    syncProfile: options.syncProfile || profileName,
    set: options.set || null,
    feat: options.feat || null,
    dirtyLimit: options.dirtyLimit == null ? 10 : Number(options.dirtyLimit),
    timeoutMs: options.timeoutMs || null,
    includeVenv: options.includeVenv !== false,
    default: true,
    featDefault: true,
  });
  const allowedProblems = options.allowedProblems || (
    options.allowUnboundVenv === true ? ['venv_binding_missing'] : []
  );
  let blocking = remoteStatusHasBlockingDrift(status, {
    allowedProblems,
  });
  if (status.sources && status.sources.length === 1) {
    const source = status.sources[0];
    const hasSelectedLocalSource = source.local && source.local.path;
    if (!hasSelectedLocalSource) {
      blocking = blocking.map(item => ({
        ...item,
        problems: item.problems.filter(problem => problem !== 'local_path_missing'),
...
function refreshBindingFromGuard(profile, guard) {
  const source = guard && guard.status && guard.status.sources && guard.status.sources.length === 1
    ? guard.status.sources[0]
    : null;
  const local = source && source.local ? source.local : {};
  const remote = source && source.remote ? source.remote : {};
  const sourceBinding = remote && remote.binding ? remote.binding : null;
  return {
    track: guard && guard.status ? guard.status.track || null : null,
    feat: guard && guard.status ? guard.status.feat || null : null,
    profile: profile.name,
    worktree_id: source ? source.id || null : null,
    repo: source ? source.repo || null : null,
    local_path: local.path || (sourceBinding ? sourceBinding.local_path || null : null),
    remote_path: remote.path || profile.source_dir || null,
    branch: local.branch || remote.effective_branch || remote.branch || (sourceBinding ? sourceBinding.branch || null : null),
    source_head: local.head || remote.effective_head || remote.head || (sourceBinding ? sourceBinding.source_head || null : null),
...
function refreshEnvProfile(config, profileName, options = {}) {
  ...
  if (execute) {
    remoteGuard = envRefreshRemoteGuard(config, name, {
      set: options.set || null,
      feat: options.feat || null,
      syncProfile: options.syncProfile || null,
      timeoutMs: options.guardTimeoutMs || options.timeoutMs || null,
      allowedProblems: options.createVenv === true
        ? (options.allowUnboundVenv === true
          ? ['venv_missing', 'venv_python_missing', 'venv_binding_missing']
          : ['venv_missing', 'venv_python_missing'])
        : null,
      allowUnboundVenv: options.allowUnboundVenv === true,
    });
    if (remoteGuard.blocking.length) {
      ...
      return {
        ...
        remote_guard: {
          status: remoteGuard.status.status,
          totals: remoteGuard.status.totals,
          blocking: remoteGuard.blocking,
        },
        next_action: 'Run sync apply for the selected track/feature first, or inspect env remote-status before refreshing this venv.',
      };
    }
    refreshBinding = refreshBindingFromGuard(profile, remoteGuard);
  }

  const command = buildVllmRefreshCommand(profile, {
    binding: refreshBinding,
    createVenv: options.createVenv === true,
  });
  const payload = {
    action: 'env_refresh',
    profile: name,
...
function handleEnvProfile(subcommand, args) {
  ...
  if (subcommand === 'refresh') {
    output(refreshEnvProfile(config, parsed.profile || null, {
      ...
      set: parsed.set || null,
      feat: parsed.feat || null,
      createVenv: parsed['create-venv'] === true,
      allowUnboundVenv: parsed['allow-unbound-venv'] === true,
      allowCrossTrack: parsed['allow-cross-track'] === true,
    }));
    return;
  }
