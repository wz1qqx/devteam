...
function materializeWorktrees(options = {}) {
  ...
  function repoConfigFor(entry) {
    return entry && entry.repo && config.repos && config.repos[entry.repo]
      ? config.repos[entry.repo]
      : {};
  }

  function remoteCommands(entry) {
    const repoConfig = repoConfigFor(entry);
    const commands = [];
    if (repoConfig.remote) {
      commands.push({
        remote: 'origin',
        url: repoConfig.remote,
        command: `git -C "${entry.abs_path}" remote set-url origin "${repoConfig.remote}"`,
      });
    }
    if (repoConfig.upstream) {
      commands.push({
        remote: 'upstream',
        url: repoConfig.upstream,
        command: `git -C "${entry.abs_path}" remote add upstream "${repoConfig.upstream}" || git -C "${entry.abs_path}" remote set-url upstream "${repoConfig.upstream}"`,
      });
    }
    return commands;
  }

  function applyRemoteCommands(entry, commands) {
    for (const item of commands) {
      if (item.remote === 'origin') {
        execFileSync('git', ['-C', entry.abs_path, 'remote', 'set-url', 'origin', item.url], {
          stdio: ['ignore', 'ignore', 'pipe'],
        });
      } else {
        try {
          execFileSync('git', ['-C', entry.abs_path, 'remote', 'add', item.remote, item.url], {
            stdio: ['ignore', 'ignore', 'pipe'],
          });
        } catch (_) {
          execFileSync('git', ['-C', entry.abs_path, 'remote', 'set-url', item.remote, item.url], {
            stdio: ['ignore', 'ignore', 'pipe'],
          });
        }
      }
  ...
  const entries = ids.map(id => {
    ...
    const command = entry.abs_source_path
      ...
      : null;
    const remotes = remoteCommands(entry);
    ...
    if (apply) {
      ...
      applyRemoteCommands(entry, remotes);
      return {
        id,
        repo: entry.repo,
        source_path: entry.abs_source_path,
