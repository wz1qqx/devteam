...
function isLocalShellProfile(profile) {
  return String(profile.ssh || '').trim() === 'sh -c' &&
    (!profile.host || String(profile.host) === 'local-shell');
}
...
function gitBindCommand(worktree, binding, profile, remotePath) {
  if (!profile.ssh || !remotePath || !binding.source_head) return null;
  const id = runId();
  const localBundlePath = `/tmp/devteam-sync-${worktree.id}-${id}.bundle`;
  const remoteBundlePath = isLocalShellProfile(profile)
    ? localBundlePath
    : `/tmp/devteam-sync-${worktree.id}-${id}.bundle`;
  const branch = binding.branch || 'devteam-sync';
  const { remote_shell: remoteShell, host } = splitSsh(profile.ssh, profile.host);
  if (!host) return null;
  const localBundle = [
    'git',
    '-C',
    shellQuote(worktree.abs_path),
    'bundle',
    'create',
    shellQuote(localBundlePath),
    'HEAD',
  ].join(' ');
  const copyBundle = isLocalShellProfile(profile)
    ? null
    : [
      'rsync -az --no-owner --no-group',
      `-e ${shellQuote(remoteShell)}`,
      shellQuote(localBundlePath),
      shellQuote(`${host}:${remoteBundlePath}`),
    ].join(' ');
  const remoteReset = [
    `mkdir -p ${shellQuote(remotePath)}`,
    `cd ${shellQuote(remotePath)}`,
    'if ! git rev-parse --git-dir >/dev/null 2>&1; then git init -q; fi',
    'if git rev-parse --verify HEAD >/dev/null 2>&1; then git reset --hard >/dev/null; fi',
    'git clean -fd >/dev/null',
    `git fetch -q ${shellQuote(remoteBundlePath)} HEAD`,
    `git checkout -B ${shellQuote(branch)} FETCH_HEAD >/dev/null`,
    'git reset --hard FETCH_HEAD >/dev/null',
    'git clean -fd >/dev/null',
    `rm -f ${shellQuote(remoteBundlePath)}`,
  ].join(' && ');
  return [
    localBundle,
    '&&',
    copyBundle ? `${copyBundle} &&` : null,
    remoteShellCommand(profile, remoteReset),
    '&&',
    `rm -f ${shellQuote(localBundlePath)}`,
  ].filter(Boolean).join(' ');
}
...
function buildSyncPlan(options = {}) {
  ...
  for (const id of ids) {
    ...
    const command = exists && remotePath
      ...
      : null;
    const sourceHead = localGitValue(worktree.abs_path, ['rev-parse', 'HEAD']);
    const binding = exists && remotePath ? {
    ...
    const bindGit = exists && remotePath && sourceHead && worktree.sync.git_bind !== false;
    const syncable = exists && remotePath && (command || bindGit);
    entries.push({
      id: worktree.id,
      repo: worktree.repo,
      local_path: worktree.abs_path,
      ...
      action: syncable ? 'sync' : (exists && remotePath ? 'noop' : 'missing'),
      reason: exists
        ? (remotePath
          ? (isRelativePatch && !command && !bindGit ? `no ${patchMode} files to sync` : null)
          : 'remote_path missing')
        : 'local worktree missing',
      command,
      binding,
      git_bind: bindGit,
      git_command: bindGit ? gitBindCommand(worktree, binding, profile, remotePath) : null,
      binding_command: syncable ? syncBindingCommand(binding, profile, remotePath) : null,
    });
  }
...
function applySyncPlan(options = {}) {
  ...
  if (execute && options.allowRemoteDrift !== true) {
    ...
    const blocking = remoteStatusHasBlockingDrift(remote_guard, {
      allowedProblems: options.allowedRemoteProblems || [
        'head_mismatch',
        'branch_mismatch',
        'local_dirty',
        'binding_dirty_mismatch',
        'remote_git_head_mismatch',
        'remote_git_branch_mismatch',
        'remote_missing',
        'remote_not_git',
      ],
      allowBoundRemoteDirty: true,
    });
    if (blocking.length) {
      return {
        profile: plan.profile,
  ...
  for (const entry of plan.entries) {
    ...
    const gitResult = entry.git_command
      ? executeCommand(entry.git_command)
      : null;
    const commandResult = (!gitResult || gitResult.status === 'passed') && entry.command
      ? executeCommand(entry.command)
      : null;
    const bindingResult = (!gitResult || gitResult.status === 'passed') &&
      (!commandResult || commandResult.status === 'passed') &&
      entry.binding_command
      ? executeCommand(entry.binding_command)
      : null;
    const finalStatus = gitResult && gitResult.status !== 'passed'
      ? 'failed'
      : (commandResult && commandResult.status !== 'passed'
      ? 'failed'
      : (bindingResult && bindingResult.status !== 'passed'
      ? 'failed'
      : 'passed'));
    const primaryResult = commandResult || gitResult || bindingResult || {
      exit_code: finalStatus === 'passed' ? 0 : 1,
      stdout: '',
      stderr: '',
      duration_ms: 0,
    };
    results.push({
      ...
      ...primaryResult,
      status: finalStatus,
      git_bind: entry.git_bind === true,
      git_command: entry.git_command || null,
